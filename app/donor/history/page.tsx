...
import { useState, useEffect } from "react"
import { 
  Calendar, MapPin, Droplets, Download, TrendingUp, Loader2 
} from "lucide-react"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import { Badge } from "@/components/ui/badge"
...
export default function DonationHistoryPage() {
  ...
  // دالة ذكية لتحديد لون ونص الـ Badge ديناميكياً بناءً على حالة الموعد القادمة من السيرفر
  const renderStatusBadge = (status: string) => {
    ...
    // الحالة الافتراضية المستلمة Completed
    return (
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <CardDescription className="font-serif">
                Your verified and upcoming donation records
              </CardDescription>
            </div>
            <Button variant="outline" onClick={() => window.print()}>
              <Download className="mr-2 h-4 w-4 font-serif" /> Export PDF
        ...
        <CardContent>
          <div className="space-y-4">
            {data.history.length === 0 ? (
                <p className="text-center text-muted-foreground py-10 font-serif">No past or upcoming donations recorded yet.</p>
            ) : (
                data.history.map((donation: any) => (
              <div
                key={donation.id}
              ...
              >
                ...
                <div className="flex items-center justify-between sm:flex-col sm:items-end">
                  ...
                  {/* التعديل الجوهري هنا: استدعاء الدالة الديناميكية لعرض الحالة الصحيحة */}
