...
export default function RewardsPage() {
  ...
  const fetchUserPoints = async () => {
    ...
    try {
      const response = await fetch("https://bloodlinknetwork2.runasp.net/api/Rewards/user-points", {
 ...
 const handleRedeem = async (rewardId: number, cost: number) => {
    ...
    try {
      const response = await fetch(`https://bloodlinknetwork2.runasp.net/api/Rewards/redeem/${rewardId}`, {
        method: "POST",
        headers: { 
